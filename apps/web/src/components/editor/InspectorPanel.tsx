...
export const InspectorPanel: React.FC = () => {
  ...
  const selectedAvatarConfig = useAvatarSelectionStore((state) => state.selectedConfig);
  const selectedAvatarConfigKey = selectedAvatarConfig
    ? `${selectedAvatarConfig.source}:${selectedAvatarConfig.id}`
    : "";
  const previousAvatarConfigKeyRef = useRef("");
  ...
  const tabs = useMemo(() => {
    const baseTabs = selectedAvatarConfig && !selectedClip
      ? [TAB_DEFS.avatarAlign, TAB_DEFS.transform]
      : getTabsForClipType(clipType as InspectorClipType | null);
    return [...baseTabs, TAB_DEFS.avatarActions];
  }, [clipType, selectedAvatarConfig, selectedClip]);
  const tabIds = useMemo(() => {
    const baseTabIds = selectedAvatarConfig && !selectedClip
      ? ["avatarAlign" as InspectorTabId, "transform" as InspectorTabId]
      : getTabIdsForClipType(clipType as InspectorClipType | null);
    return [...baseTabIds, "avatarActions" as InspectorTabId];
  }, [clipType, selectedAvatarConfig, selectedClip]);
  ...
  useEffect(() => {
    if (
      selectedAvatarConfigKey &&
      selectedAvatarConfigKey !== previousAvatarConfigKeyRef.current &&
      !selectedClip
    ) {
      previousAvatarConfigKeyRef.current = selectedAvatarConfigKey;
      setInspectorActiveTab("avatarAlign");
      return;
    }
    previousAvatarConfigKeyRef.current = selectedAvatarConfigKey;
    ...
    if (
      ...
      !selectedClip &&
      inspectorActiveTab !== "avatarAlign" &&
      inspectorActiveTab !== "transform" &&
      inspectorActiveTab !== "avatarActions"
    ) {
      setInspectorActiveTab("avatarAlign");
      return;
    }
      ...
      <div className="px-4 pt-3">
        {selectedClip ? (
          <InspectorTabErrorBoundary key={activeTab}>
            ...
            <InspectorTabPanel tab="avatarActions" active={activeTab}>
              <AvatarActionConsole clip={selectedTimelineClip} />
            </InspectorTabPanel>

          </InspectorTabErrorBoundary>
        ) : selectedAvatarConfig ? (
          activeTab === "avatarAlign" ? (
            <AvatarConfigInspector mode="align" />
          ) : activeTab === "transform" ? (
            <AvatarConfigInspector mode="transform" />
          ) : (
            <InspectorTabErrorBoundary key={activeTab}>
              <InspectorTabPanel tab="avatarActions" active={activeTab}>
                <AvatarActionConsole />
