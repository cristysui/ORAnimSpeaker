...
const isAvatarGeneratedClip = (clip: PreviewClip): boolean => {
  return clip.metadata?.kind === "avatar-generated";
};

const isAvatarGeneratedSequenceClip = (clip: PreviewClip): boolean => {
  return (
    clip.metadata?.kind === "avatar-generated" &&
...
const hasAvatarActionOverlayAtTime = (tracks: Track[], time: number): boolean => {
  return tracks.some((track) => {
    if (track.hidden || (track.type !== "video" && track.type !== "image")) return false;
    return track.clips.some((clip) => isAvatarActionOverlayClip(clip) && clipIsActiveAtTime(clip, time));
  });
};
...
const withPlaybackTimeout = <T,>(promise: Promise<T>, timeoutMs: number): Promise<T> => {
  return Promise.race([
    ...
    new Promise<T>((_, reject) => {
      window.setTimeout(() => reject(new Error(`Playback step timed out after ${timeoutMs}ms`)), timeoutMs);
    }),
  ]);
};
...
export const Preview: React.FC = () => {
    ...
    async (
      ...
      if (activeStartClip) {
        await withPlaybackTimeout(
          loadVideoForClip(activeStartClip.clip, activeStartClip.mediaItem),
          1200,
        ).catch((error) => {
          console.warn("[Preview] Initial video warmup skipped:", error);
        });
      }
      ...
      await withPlaybackTimeout(audioGraph.resume(), 1200).catch((error) => {
        console.warn("[Preview] Audio resume skipped:", error);
      });
      audioGraph.seekTo(startPosition);
      await withPlaybackTimeout(masterClock.play(), 1200);
      ...
      const drawFrame = async () => {
        ...
        const sourceDimensions = {
          width: mediaItem.metadata.width || video.videoWidth || canvas.width,
          height: mediaItem.metadata.height || video.videoHeight || canvas.height,
        };
        const vidstabEng = getVidstabEngine();
        if (
  ...
  useEffect(() => {
    ...
    const playbackStartPosition = startPositionRef.current;
      ...
      tracks.forEach((track, originalIndex) => {
        if (
          ...
          let activeSequenceFrame:
            | { clip: (typeof tracks)[0]["clips"][0]; track: (typeof tracks)[0]; trackIndex: number }
            | null = null;
          for (const clip of track.clips) {
            if (
              time >= clip.startTime &&
              ...
              if (track.type === "image" && isAvatarGeneratedSequenceClip(clip)) {
                if (!activeSequenceFrame || clip.startTime >= activeSequenceFrame.clip.startTime) {
                  activeSequenceFrame = { clip, track, trackIndex: originalIndex };
                }
                continue;
              }
          ...
          if (activeSequenceFrame) {
            results.push(activeSequenceFrame);
          }
        }
      });
    ...
    const getImageBitmapForClip = async (
      clip: (typeof timelineTracksRef.current)[0]["clips"][0],
    ): Promise<ImageBitmap | null> => {
      const cachedBitmap = imageBitmapCacheRef.current.get(clip.mediaId);
      ...
      try {
        ...
        console.warn(
          `[Preview] Failed to load image clip ${clip.id}:`,
          error,
        );
        return null;
      }
      ...
      const processMultiTrackFrame = async () => {
        ...
        try {
          ...
          for (const { clip, track } of sortedClips) {
            ...
            if (track.type === "image") {
              const cachedBitmap = await getImageBitmapForClip(clip);
              if (cachedBitmap) {
          ...
          if (
            ...
            const clipToTrackIndex = new Map(
              activeClips.map(({ clip, trackIndex }) => [clip.id, trackIndex]),
            );
      ...
      >
        ...
        <div className="flex items-center gap-1 mx-auto">
          ...
          <button
            onClick={() => {
              if (!isPlaying) {
                getMasterClock().getAudioContext().resume().catch(() => {});
                getRealtimeAudioGraph().resume().catch(() => {});
              }
              togglePlayback();
            }}
