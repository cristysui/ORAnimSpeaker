...
import {
  ...
  Type, Shapes, Wand2, LayoutTemplate, Zap, Shuffle, UserRound,
} from "lucide-react";
import {
  BACKGROUND_PRESETS,
...
const DEFAULT_BACKGROUND_DURATION = 5;

const getLongestTrackDuration = (
  tracks: Array<{ clips: Array<{ startTime: number; duration: number }> }>,
): number => {
  return tracks.reduce((longest, track) => {
    ...
    return Math.max(longest, trackEnd);
  }, 0);
};
...
const ASSETS_TABS: ReadonlyArray<{
...
}> = [
  {
    value: "avatar",
    label: "角色动画",
    description: "配置人物视频或序列帧后，在音频轨上方生成角色动画。",
  },
  {
    value: "backgrounds",
    label: "Backgrounds",
    description: "Add generated solid, gradient, mesh, and pattern backgrounds.",
  },
] as const;
    ...
    async (preset: BackgroundPreset) => {
      ...
      try {
        ...
        if (result.success && result.actionId) {
          const stateBeforeTrack = useProjectStore.getState();
          const timelineDuration = Math.max(
            getLongestTrackDuration(stateBeforeTrack.project.timeline.tracks),
            stateBeforeTrack.project.timeline.duration || 0,
            DEFAULT_BACKGROUND_DURATION,
          );
          const existingTrackIds = new Set(
            stateBeforeTrack.project.timeline.tracks.map((track) => track.id),
          );
          const trackResult = await stateBeforeTrack.addTrack(
            "image",
            stateBeforeTrack.project.timeline.tracks.length,
          );
          if (!trackResult.success) return;

          const stateAfterTrack = useProjectStore.getState();
          const backgroundTrack = stateAfterTrack.project.timeline.tracks.find(
            (track) => !existingTrackIds.has(track.id) && track.type === "image",
          );
          if (!backgroundTrack) return;

          stateAfterTrack.renameTrack(backgroundTrack.id, "Background");

          const clipResult = await useProjectStore
            .getState()
            .addClip(backgroundTrack.id, result.actionId, 0);
          if (!clipResult.success) return;

          const createdClip = useProjectStore
            .getState()
            .project.timeline.tracks.find((track) => track.id === backgroundTrack.id)
            ?.clips.find((clip) => clip.mediaId === result.actionId);

          if (createdClip && createdClip.duration !== timelineDuration) {
            await useProjectStore
              .getState()
              .trimClip(createdClip.id, 0, timelineDuration);
          }
        }
      } catch (error) {
