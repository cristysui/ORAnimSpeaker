...
export class MasterTimelineClock {
  ...
  private startPerformanceTime: number = 0;
    ...
    const elapsed =
      this.audioContext.state === "running"
        ? (this.audioContext.currentTime - this.startAudioContextTime) *
          this.playbackRate
        : ((performance.now() - this.startPerformanceTime) / 1000) *
          this.playbackRate;
  ...
  async play(): Promise<void> {
    ...
    if (this.audioContext.state === "suspended") {
      this.audioContext.resume().then(() => {
        if (this.state !== "playing") return;
        const currentTime = this.getPerformanceBackedCurrentTime();
        this.startTimelineTime = currentTime;
        this.startAudioContextTime = this.audioContext.currentTime;
        this.startPerformanceTime = performance.now();
      }).catch(() => {});
    }
  ...
  private getPerformanceBackedCurrentTime(): number {
    const elapsed =
      ((performance.now() - this.startPerformanceTime) / 1000) *
      this.playbackRate;
    let time = this.startTimelineTime + elapsed;
