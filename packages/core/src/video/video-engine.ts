...
export class VideoEngine {
      ...
      if (track.type === "video" || track.type === "image") {
        ...
        for (const clip of clips) {
          ...
          if (mediaItem.type === "image") {
            ...
            const decodeDimensions = this.getClipDecodeDimensions(
              clipInfo.transform,
              mediaItem,
              settings.width,
              settings.height,
            );
            const decodeCacheId = this.getClipDecodeCacheId(
              mediaItem.id,
            ...
            if (!bitmap) {
              ...
              bitmap = await this.decodeFrameWithMediaBunny(
                ...
                decodeDimensions.width,
                decodeDimensions.height,
                useStabilizedBlob ? `stabilized:${decodeCacheId}` : decodeCacheId,
              );
            }
            if (!bitmap) {
              const vidstabForDecode = getVidstabEngine();
  ...
  private getClipDecodeDimensions(
    transform: Transform,
    mediaItem: MediaItem,
    ...
    const metadataWidth = mediaItem.metadata?.width;
    const metadataHeight = mediaItem.metadata?.height;
    const width = Number.isFinite(metadataWidth) && metadataWidth && metadataWidth > 0
      ? metadataWidth
      : fallbackWidth;
    const height = Number.isFinite(metadataHeight) && metadataHeight && metadataHeight > 0
      ? metadataHeight
      : fallbackHeight;
    return { width, height };
  }

  private getClipDecodeCacheId(
    mediaId: string,
    decodeWidth: number,
    transform: Transform,
  ): string {
    return (transform.fitMode ?? "none") === "none"
      ? `${mediaId}:natural:${Math.round(decodeWidth)}`
      : mediaId;
  }
